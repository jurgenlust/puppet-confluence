...
# manage the Tomcat instance
	tomcat::webapp { "${user}" :
		...
		java_opts => "-server -Dorg.apache.jasper.runtime.BodyContentImpl.LIMIT_BUFFER=true -Dmail.mime.decodeparameters=true -Xms${memory} -Xmx${memory} -XX:PermSize=${permsize} -XX:MaxPermSize=${permsize} -Djava.awt.headless=true",
